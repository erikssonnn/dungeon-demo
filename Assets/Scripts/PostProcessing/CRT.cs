    ...
    private void OnRenderImage(RenderTexture src, RenderTexture dest) {
        ...
        if (whiteNoiseTimeLeft <= 0) {
            if (Random.Range(0, 1000) < whiteNoiseFrequency) {
                material.SetInt(_WhiteNoiseOnOff, 1);
                whiteNoiseTimeLeft = whiteNoiseLength;
            } else {
                material.SetInt(_WhiteNoiseOnOff, 0);
            }
        }
        //////

        material.SetInt(_LetterBoxOnOff, isLetterBox ? 0 : 1);
        //material.SetInteger(_LetterBoxEdgeBlurOnOff, isLetterBoxEdgeBlur ? 0 : 1); 
        material.SetInt(_LetterBoxType, (int)letterBoxType);

        material.SetInt(_ScanlineOnOff, isScanline ? 1 : 0);
        ...
        material.SetInt(_FilmDirtOnOff, isFilmDirt ? 1 : 0);
        ...
        material.SetInt(_DecalTexOnOff, isDecalTex ? 1 : 0);
